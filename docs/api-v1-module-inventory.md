...
| `pinned_facts` | `/api/v1/pinned-facts` | implemented in Phase 10 | Owns pinned fact definitions, current-value summaries, candidate review, conflict views, and append-only value history. |
| `changes` | `/api/v1/changes` | implemented in Phase 10 | Owns append-only change events, detail reads, and per-user read-state markers for processing, pinned-facts, and correction activity. |
| `corrections` | `/api/v1/corrections` | implemented in Phase 10 | Owns correction submission, verification, rejection, and verified-evidence handoff into pinned-facts and chat. |
...
The follow-on Phase 10 slice implemented chat, pinned-facts, changes, and corrections on top of the shared retrieval contracts without adding parallel retrieval semantics.
