...
logger = get_logger("ragdoll.workers.document_vector_worker")
...
def install_worker_signal_handlers(stop_event: threading.Event) -> dict[int, Any]:
    ...
    def _handle_shutdown(signum: int, _frame: FrameType | None) -> None:
        logger.info("Received shutdown signal signum=%s; stopping document-vector worker.", signum)
        stop_event.set()
...
def run_document_worker(
    *,
    ...
    logger.info(
        "Starting document-vector worker poll_interval_seconds=%s",
        idle_poll_interval,
    )
    ...
    while not active_stop_event.is_set():
        try:
            ...
            logger.exception("Document-vector worker iteration failed; continuing after backoff.")
            processed = False
    ...
    logger.info("Document-vector worker stopped.")
