...
def create_chat_session(session: Session, subject: str, *, space_scope: SpaceScope) -> ChatSession:
    owner_user_id = UUID(subject)
    space = resolve_single_owned_space(session, owner_user_id, space_scope)
    chat_session = ChatSession(space_id=space.id, owner_user_id=owner_user_id, title="New chat")
