...
def _detected_from_result(result: SearchResult) -> DetectedCandidate | None:
    ...
    return DetectedCandidate(
        ...
        idempotency_key=f"{source_document_id}:{_value_signature(snapshot)}",
    )
...
def _apply_fact_value(
    ...
    *,
    ...
    old_evidence = _evidence_from_raw(fact.evidence)
    ...
    fact.evidence = _raw_evidence(evidence)
    fact.status = status
...
def create_pinned_fact(session: Session, subject: str, *, space_scope: SpaceScope, payload) -> PinnedFact:
    ...
    fact = PinnedFact(
        ...
        evidence=_raw_evidence(payload.evidence),
...
def update_pinned_fact(session: Session, fact: PinnedFact, *, payload) -> PinnedFact:
...
def _detect_candidates(results: list[SearchResult], current: ValueSnapshot | None) -> tuple[str, list[DetectedCandidate], list[PinnedFactEvidence]]:
    evidence_items = []
    ...
    for result in results:
        ...
        detected_by_value.setdefault(key, detected)
...
def recheck_pinned_fact(
    ...
    *,
    ...
    decision, detected_items, supporting_evidence = _detect_candidates(results, current)
    ...
    if decision == "unknown":
        ...
        if detected.confidence >= 0.9 and detected.evidence:
            ...
            record_change_event(
                session,
                space_id=fact.space_id,
                event_type="pinned_fact_updated",
                title=f"Pinned fact updated: {fact.title}",
                summary=f"{fact.title} updated automatically.",
                actor_user_id=actor_user_id,
                pinned_fact_id=fact.id,
                payload={"value_kind": detected.value.kind},
            )
        else:
            _persist_candidate(session, fact=fact, detected=detected, status="pending")
